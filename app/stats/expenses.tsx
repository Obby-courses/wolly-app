  ...
  const loadStats = async () => {
    ...
    try {
      ...
      const cData = await TransactionRepository.getCategoryDistribution(timeRange, 'out', baseDate, {
        // Need to update getCategoryDistribution to support domain_filter if we want strict filtering here, 
        // but for now we can filter the results in JS or just show all and filter by domain if needed.
      });
      ...
      <ScrollView contentContainerStyle={styles.scrollContent} showsVerticalScrollIndicator={false}>
        {loading ? <ActivityIndicator size="large" color={COLORS.primary} style={{ marginTop: 50 }} /> : (
          <>
            ...
            <View style={styles.card}>
              <View style={styles.cardHeader}>
                <Text style={styles.cardTitle}>Transazioni</Text>
                <View style={styles.sortContainer}>
                  ...
                  <Pressable onPress={() => setSortBy('amount_desc')} style={[styles.sortBtn, sortBy === 'amount_desc' && styles.sortBtnActive]}>
                    <Ionicons name="trending-down-outline" size={16} color={sortBy === 'amount_desc' ? '#FFF' : COLORS.secondary} />
                  </Pressable>
                  <Pressable onPress={() => setSortBy('amount_asc')} style={[styles.sortBtn, sortBy === 'amount_asc' && styles.sortBtnActive]}>
                    <Ionicons name="trending-up-outline" size={16} color={sortBy === 'amount_asc' ? '#FFF' : COLORS.secondary} />
                  </Pressable>
                </View>
              </View>
                ...
                transactions.map((tx, idx) => (
                  ...
                  >
                    <View style={styles.txInfo}>
                      <Text style={styles.txDesc} numberOfLines={1}>{tx.description || 'Senza descrizione'}</Text>
                      <Text style={styles.txMeta}>{tx.date} • {tx.subcategory_key}</Text>
                    </View>
                    <View style={{ flexDirection: 'row', alignItems: 'center' }}>
                      <Text style={[styles.txAmount, tx.direction === 'in' ? styles.txIn : styles.txOut]}>
...
const styles = StyleSheet.create({
  ...
  emptyChart: { alignItems: 'center', justifyContent: 'center', backgroundColor: '#F9FAFB', borderRadius: 16, borderStyle: 'dashed', borderWidth: 1, borderColor: COLORS.border },
  ...
  chartLabelText: { fontSize: 10, color: COLORS.secondary, fontFamily: TYPOGRAPHY.fontFamily },
